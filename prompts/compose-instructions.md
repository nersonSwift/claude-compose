...
**Plugins** — reusable Claude Code extensions from marketplaces or local paths. Configured in `plugins` array. Marketplace plugins are auto-installed; local plugins are loaded via `--plugin-dir`.
...
**Build** — the process of syncing resources from workspaces/local config and resolving plugins into the workspace's `.claude/` directory and `.claude/claude-compose/mcp.json`. Runs automatically at launch when changes are detected.

**Global config** — `~/.claude-compose/global.json` is an optional file with the same schema as `claude-compose.json`. It is auto-applied to **all** workspaces. Useful for plugins, MCP servers, agents, or skills you always want available. Paths in `resources` resolve relative to `~/.claude-compose/`. Build priority: built-in → **global** → workspace workspaces → local resources. Global env files are loaded without prefix; local env files with the same keys override them.
...
- `.claude/claude-compose/mcp.json` is **generated** at build time — manual edits will be overwritten
- `.claude/claude-compose/manifest.json` and `.claude/claude-compose/hash` are internal state files
...
3. **Use env files for secrets** — never hardcode API keys in `claude-compose.json` or `.claude/claude-compose/mcp.json`. Use `${VAR}` references in MCP configs and provide values in `.env.secrets.json` (gitignored).

4. **Plugins for reuse** — if you use the same tools across multiple workspaces, install marketplace plugins instead of duplicating config.
