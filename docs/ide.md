...
Adds `claudeCode.claudeProcessWrapper` to your IDE's `settings.json`:
...
Creates `{name}.code-workspace` (using the `name` field from your config) with all project folders:
...
1. Run `claude-compose ide`
2. Open the generated `.code-workspace` file in your IDE
3. Start a Claude session — it automatically uses your compose config

## How Wrap Mode Works

When your IDE starts a Claude session:

1. The IDE calls `claude-compose-wrapper` instead of `claude` directly
2. The wrapper runs `claude-compose wrap /path/to/claude [ide-args...]`
...
4. System prompts from the IDE and claude-compose are merged
5. `exec claude` with all composed arguments

This means IDE features (streaming, output formatting) work normally while getting all your workspace configuration.

## Removing Integration

1. Open your IDE settings (VS Code: Cmd+Shift+P → "Preferences: Open Settings (JSON)")
2. Delete the `claudeCode.claudeProcessWrapper` line
3. Optionally delete the `.code-workspace` file

## Troubleshooting

**Claude works in terminal but not in IDE:**
...
- Check that `jq` is available in your IDE's shell environment

**Workspace file not updating:**
- Re-run `claude-compose ide` to regenerate
